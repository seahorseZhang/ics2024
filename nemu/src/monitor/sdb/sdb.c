...
static struct {
  ...
  int (*handler)(char *);
    ...
    {"info", "display some information about registers or watchpoints",
     cmd_info},
...
static int cmd_mem(char *args) {
  ...
  paddr_t guest_addr = (paddr_t)strtoul(hex_str, NULL, 16);
  for (int index = 0; index < step; index++) {
    uint8_t *phy_addr = guest_to_host(guest_addr);
    guest_addr += sizeof(int8_t);
    printf("%02x ", *phy_addr);
  }
  printf("\n");
  return 0;
